...
<template>
  <div class="myTable">
    ...
    <el-table :data="tableData" @select="select" @select-all="selectAll" @row-dblclick="rowDblclick" :highlight-current-row="tableConfig.isHigh">
      ...
      <el-table-column v-for="(item,index) in tableConfig.colConfig" :fixed="item.fixed" :prop="item.field" :width="item.width"
        ...
        <template slot-scope="scope">
          ...
          <!-- 多文字标签 -->          
          <span v-else-if="item.type == 'btnTextArrStatus'">
            <el-button v-for="( item2, i ) in JSON.parse(item.field)" :disabled="!setWorkStatus(scope.row.workStatus)[i]" size="small" type="text" :key="item2.key" @click="buttonFunction2(scope.row,i) ">{{item2.field}}</el-button>
          </span>
          <!-- 未完待续 -->
        </template>
      </el-table-column>
    ...
    <div class="my-pagination">
      <el-pagination v-if="tableConfig.isPage" v-show="tableConfig.total>0" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="tableConfig.pageNo"
        :page-sizes="tableConfig.pageSizes|| [10, 20, 30, 40, 50, 100] " :page-size="tableConfig.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="tableConfig.total">
      </el-pagination>
    </div>
    <!-- loading -->
  ...
  export default {
    ...
    mounted(){
      this.$nextTick(function(){
         document.getElementsByClassName('el-dialog')[0].style.marginTop = '40px';
      })
    },
    data() {
      return {
    ...
    methods: {
      setWorkStatus(code = 0){//[开始 1， 挂起 2， 取消 4]
        // return [1,1,1]
        code=parseInt(code);
        let result;
          switch( code ){
      ...
      buttonFunction2(row,i) {
        this.$emit("buttonFunction2",{row,i});
      }
    },
    watch:{
      'tableData':function(){
