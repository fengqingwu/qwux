...
import pLToastBody from "../components/SpaceDisk/pLToastBody";
export default {
  data() {
    return {
      toastType: "",
  ...
  methods: {
    ...
    eidtOperation() {
      ...
      let params = ToastBody.ruleForm;
        editEmpty({ Vue: this, params: this.setAttr(params) }).then(data => {
        this.$message({
    ...
    addOperation(flag) {
      ...
      let params = ToastBody.ruleForm;
      delete params["id"];
      addEmpty({ Vue: this, params: this.setAttr(params) }).then(data => {
        this.$message({
          message: data.message,
          type: "success"
        });
